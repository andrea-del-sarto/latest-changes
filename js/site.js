    ...
    d3.xml('https://api.openstreetmap.org/api/0.6/map?bbox=' +
        ...
        bounds.getNorthEast().lat // + ',' +
           ...
           .attr('href', function(d) {
               return 'https://www.openstreetmap.org/user/' + d.user;
           });
           ...
           rl.append('div').attr('class', 'changeset').each(function(d) {
               var t = this;
               d3.xml('https://www.openstreetmap.org/api/0.6/changeset/' + d.id)
                   .on('load', function(xml) {
                       d3.select(t).html(
                           '<a href="https://www.openstreetmap.org/changeset/' + d.id + '" target="_blank">' +
